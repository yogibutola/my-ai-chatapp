...
constructor() {
  ...
  async uploadFile(event: Event): Promise<void> {
      ...
      const baseURL = "https://capitalreport-209368501132.europe-west1.run.app";
      const UPLOAD_ENDPOINT = baseURL + "/api/v1/upload-files/";
...
//           }
  ...
  async sendMessage(event: Event): Promise<void> {
    ...
    const URL = `http://localhost:8000/api/v1/ask_question/?document_name=${encodeURIComponent(file?.name || '')}&query=${encodeURIComponent(query)}`;
    try {
      const activeDocs = this.selectedFiles();
      const names = activeDocs.map(f => f.name).join(', ');
      const baseURL = 'https://capitalreport-209368501132.europe-west1.run.app';
      const URL = baseURL + `/api/v1/ask_question/?documents=${encodeURIComponent(names)}&query=${encodeURIComponent(query)}`;
